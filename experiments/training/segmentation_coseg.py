...
from mesh_opformer.dataset.utils import parse_segmentation_label_coseg
...
def parse_data_paths(folder: str) -> Dict[str, str]:
    ...
    `coseg-segmentation` dataset. The following sources are used:

    MeshCNN (Hanocka et al., 2019):
        'https://www.dropbox.com/s/34vy4o5fthhz77d/coseg.tar.gz'

    PD-Meshnet (Milano et al., 2020):
        'https://www.dropbox.com/s/zro2o7y42wag5yp/coseg.zip'

    The above may be identical. Although the datasets are pre-split
    into training and test sets, these are random 85%-15% splits
    from the authors. We can test on the dataset and report an
    average on 5 random train-test splits.

    The `Alien` dataset contains meshes of ~size:
        n_v = 752 
        n_e = 2250 
        n_f = 1500
...
def recombine_and_resplit(
    ...
    """
    dataset = {}
    dataset.update(train_set)
    dataset.update(test_set)

    dataset_items = [item for item, _ in dataset.items()]
    ...
    train_set_upd = {item: dataset[item] for item in x_train}
    test_set_upd = {item: dataset[item] for item in x_test}

    return train_set_upd, test_set_upd
...
def initialize_map_datasets(cfg):
    ...
    # Recombine and split if necessary
    if cfg["dataset"]["resplit"]["flag"] is True:
        train_set, test_set = recombine_and_resplit(
            train_set, test_set, **cfg["dataset"]["resplit"]["ratio"]
        )
...
def calculate_class_weights(y_train: List[str], num_classes: int) -> torch.Tensor:
    ...
    """
    all_counts = [
        np.bincount(parse_segmentation_label_coseg(path), minlength=num_classes)
        for path in y_train
    ]
...
def main():
    ...
    save_path = os.path.join(args.out, "segmentation_coseg_"+cfg["wandb"]["name"]+"ckpt_{}.pth".format(epoch))
